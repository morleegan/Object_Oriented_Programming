...
f.close()"""
############
url1 = "http://localhost:8080/thalia/seating"
r1 = requests.get(url=url1)
out1 = r1.json()
# sort the json files
out1 = sorted(out1, key=lambda x: x['sid'])
cmp1 = sorted(cmp1, key=lambda x: x['sid'])
if np.all([sorted(a.items()) == sorted(b.items()) for a, b in zip(out1, cmp1)]):
    scores += 1
...
with open("cmp_files/out3.txt") as f:
    cmp3 = json.load(f)
f.close()
url3 = "http://localhost:8080/thalia/shows"
...
with open("cmp_files/out4.txt") as f:
    cmp4 = json.load(f)
f.close()
url4 = "http://localhost:8080/thalia/shows"
...
f.close()
cmp5[0]['wid'] = wid1
url5 = "http://localhost:8080/thalia/shows"
...
f.close()
cmp6['wid'] = wid1
url6 = join("http://localhost:8080/thalia/shows", wid1)
...
with open("cmp_files/out7.txt") as f:
    cmp7 = f.read()
f.close()
url7 = join("http://localhost:8080/thalia/shows", wid1)
...
f.close()
cmp8['wid'] = wid1
url8 = join("http://localhost:8080/thalia/shows", wid1)
...
for a in cmp9:
    ...
    if a['section_name'] == 'Front left':
        a['sid'] = sid3
url9 = "http://localhost:8080/thalia/shows/" + wid1 + "/sections"
...
cmp10['wid'] = wid1
cmp10['sid'] = sid1
url10 = "http://localhost:8080/thalia/shows/" + wid1 + "/sections/" + sid1
    ...
    if chk10:
        ...
        for a, b in zip(sorted(seating_o10, key=lambda x: x['row']), sorted(seating_c10, key=lambda x: x['row'])):
            if a['row'] != b['row']:
                chk10 = False
                break
            seats_o10 = sorted(a['seats'], key=lambda x: x['cid'])
            seats_c10 = sorted(b['seats'], key=lambda x: x['cid'])
...
if chk10:
    scores += 1
    print("10.Validate seating in section %s for show %s ( GET /shows/%s/sections/%s ): (pass)" % (
        sid1, wid1, wid1, sid1))
else:
    print("10.Validate seating in section %s for show %s ( GET /shows/%s/sections/%s ): (fail)" % (
        sid1, wid1, wid1, sid1))
# ************************case 10 end****************************************************
...
url11 = "http://localhost:8080/thalia/seating?show=" + wid1 + "&section=" + sid1 + "&count=5"
...
#####
chk11 = False
if cmp11['wid'] == out11['wid'] and cmp11['sid'] == out11['sid'] and cmp11['section_name'] == out11['section_name'] and \
                cmp11['starting_seat_id'] == out11['starting_seat_id'] and cmp11['status'] == out11['status'] and len(
    cmp11['seating']) == len(out11['seating']):
...
if chk11:
    scores += 1
    print(
        "11.Request a number of contiguous seats that cannot be found in section %s (\"Front left\") ( GET /seating?show=%s&section=%s&count=5 ): (pass)" % (
            sid1, wid1, sid1))
else:
    print(
        "11.Request a number of contiguous seats that cannot be found in section %s (\"Front left\") ( GET /seating?show=%s&section=%s&count=5 ): (fail)" % (
            sid1, wid1, sid1))
# ************************case 11 end****************************************************
...
for s_ in cmp12['seating'][0]['seats']:
    if s_['seat'] == '1':
        ...
        s_['cid'] = cid4
url12 = "http://localhost:8080/thalia/seating?show=" + wid1 + "&section=" + sid1 + "&count=4"
...
for s_ in out12['seating'][0]['seats']:
    if s_['seat'] == '1':
        ...
        s_['cid'] = cid4
chk12 = False
if cmp12['wid'] == out12['wid'] and cmp12['sid'] == out12['sid'] and cmp12['section_name'] == out12['section_name'] and \
                cmp12['starting_seat_id'] == out12['starting_seat_id'] and cmp12['status'] == out12['status'] and cmp12[
    'total_amount'] == out12['total_amount']:
...
if chk12:
    scores += 1
    print(
        "12.Request a number of contiguous seats that can be accomodated by section %s ( GET /seating?show=%s&section=%s&count=4 ): (pass)" % (
            sid1, wid1, sid1))
else:
    print(
        "12.Request a number of contiguous seats that can be accomodated by section %s ( GET /seating?show=%s&section=%s&count=4 ): (fail)" % (
            sid1, wid1, sid1))
# ************************case 12 end****************************************************
...
for s_ in cmp13['seating'][0]['seats']:
    if s_['seat'] == '2':
        ...
        s_['cid'] = cid4
url13 = "http://localhost:8080/thalia/seating?show=" + wid1 + "&section=" + sid1 + "&count=3" + "&starting_seat_id=" + cid2
...
##########
chk13 = False
if cmp13['wid'] == out13['wid'] and cmp13['sid'] == out13['sid'] and cmp13['section_name'] == out13['section_name'] and \
                cmp13['starting_seat_id'] == out13['starting_seat_id'] and cmp13['status'] == out13['status'] and cmp13[
    'total_amount'] == out13['total_amount']:
...
if chk13:
    scores += 1
    print(
        "13.Request a number of contigious seats starting with a specific seat in the section (GET /seating?show=%s&section=%s&count=3&starting_seat_id=%s): (pass)" % (
            wid1, sid1, cid2))
else:
    print(
        "13.Request a number of contigious seats starting with a specific seat in the section (GET /seating?show=%s&section=%s&count=3&starting_seat_id=%s): (fail)" % (
            wid1, sid1, cid2))
# ************************case 13 end****************************************************
...
for s_ in cmp14['seating'][0]['seats']:
    if s_['seat'] == '1':
        ...
        s_['cid'] = cid8
url14 = "http://localhost:8080/thalia/seating?show=" + wid1 + "&section=" + sid1 + "&count=4" + "&starting_seat_id=" + cid2
...
############
chk14 = False
if cmp14['wid'] == out14['wid'] and cmp14['sid'] == out14['sid'] and cmp14['section_name'] == out14['section_name'] and \
                cmp14['starting_seat_id'] == out14['starting_seat_id'] and cmp14['status'] == out14['status'] and cmp14[
    'total_amount'] == out14['total_amount']:
...
if chk14:
    scores += 1
    print(
        "14.Test another request for contiguous seats starting with a specific seat ( GET /seating?show=%s&section=%s&count=4&starting_seat_id=%s ): (pass)" % (
            wid1, sid1, cid2))
else:
    print(
        "14.Test another request for contiguous seats starting with a specific seat ( GET /seating?show=%s&section=%s&count=4&starting_seat_id=%s ) (fail)" % (
            wid1, sid1, cid2))
# ************************case 14 end****************************************************
...
f.close()
cmp15['wid'] = wid1
url15 = "http://localhost:8080/thalia/orders"
...
chk15 = False
if cmp15['wid'] == out15['wid'] and cmp15['order_amount'] == out15['order_amount'] and len(cmp15['tickets']) == len(
        out15['tickets']):
...
url16 = "http://localhost:8080/thalia/orders"
...
########
chk16 = False
if cmp16[0]['wid'] == out16[0]['wid'] and cmp16[0]['oid'] == out16[0]['oid'] and cmp16[0]['date_ordered'] == out16[0][
    'date_ordered'] and cmp16[0]['order_amount'] == out16[0]['order_amount'] and cmp16[0]['number_of_tickets'] == \
        out16[0]['number_of_tickets']:
...
url17 = join("http://localhost:8080/thalia/orders", oid1)
...
#####
chk17 = False
if cmp17['wid'] == out17['wid'] and cmp17['oid'] == out17['oid'] and cmp17['date_ordered'] == out17['date_ordered'] and \
                cmp17['order_amount'] == out17['order_amount']:
...
f.close()
cmp18['wid'] = wid1
url18 = "http://localhost:8080/thalia/orders"
...
chk18 = False
if cmp18['wid'] == out18['wid'] and cmp18['order_amount'] == out18['order_amount'] and len(cmp18['tickets']) == len(
        out18['tickets']):
...
cmp19['wid'] = wid1
cmp19['sid'] = sid2
url19 = "http://localhost:8080/thalia/shows/" + wid1 + "/sections/" + sid2
...
if chk19:
    scores += 1
    print(
        "19.Request seating information for section %s (\"Front center\") for show %s ( GET /shows/%s/sections/%s ): (pass)" % (
            sid2, wid1, wid1, sid2))
else:
    print(
        "19.Request seating information for section %s (\"Front center\") for show %s ( GET /shows/%s/sections/%s ): (fail)" % (
            sid2, wid1, wid1, sid2))
# ************************case 19 end****************************************************
...
cmp20['wid'] = wid1
data20 = {'data': json20}
url20 = "http://localhost:8080/thalia/orders"
...
chk20 = False
if cmp20['wid'] == out20['wid'] and cmp20['order_amount'] == out20['order_amount'] and len(cmp20['tickets']) == len(
        out20['tickets']):
...
for s_ in cmp21['seating'][1]['seats']:
    if s_['seat'] == '1':
        ...
        s_['cid'] = cid8
url21 = "http://localhost:8080/thalia/shows/" + wid1 + "/sections/" + sid1
    ...
    if chk21:
        ...
        for a, b in zip(sorted(seating_o21, key=lambda x: x['row']), sorted(seating_c21, key=lambda x: x['row'])):
            if a['row'] != b['row']:
                chk21 = False
                break
            seats_o21 = sorted(a['seats'], key=lambda x: x['cid'])
            seats_c21 = sorted(b['seats'], key=lambda x: x['cid'])
            for a_, b_ in zip(seats_o21, seats_c21):
                if seats_o21[0]['cid'] != seats_o21[0]['cid'] or seats_o21[1]['cid'] != seats_o21[1]['cid'] or a_[
                    'seat'] != b_['seat'] or a_['status'] != b_['status']:
                    chk21 = False
                    break
                chk21 = True
            if chk21 == False:
...
if chk21:
    scores += 1
    print("21.Validate the seating in section %s for show %s ( GET /shows/%s/sections/%s ): (pass)" % (
        sid1, wid1, wid1, sid1))
else:
    print("21.Validate the seating in section %s for show %s ( GET /shows/%s/sections/%s ): (fail)" % (
        sid1, wid1, wid1, sid1))
# ************************case 21 end****************************************************
...
url22 = "http://localhost:8080/thalia/orders?start_date=" + start_date + "&end_date=" + end_date
...
for a, b in zip(orders_o22, orders_c22):
    if a['wid'] == b['wid'] and a['oid'] == b['oid'] and a['date_ordered'] == b['date_ordered'] and a['order_amount'] == \
            b['order_amount'] and a['number_of_tickets'] == b['number_of_tickets']:
...
f.close()
data23 = {'data': json23}
url23 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations"
...
f.close()
data24 = {'data': json24}
url24 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations"
...
cmp25['did'] = did1
cmp25['wid'] = wid1
url25 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations/" + did1
...
#############
chk25 = False
if out25['did'] == cmp25['did'] and out25['wid'] == cmp25['wid'] and out25['count'] == cmp25['count'] and out25[
    'status'] == cmp25['status'] and len(out25['tickets']) == len(cmp25['tickets']):
...
cmp26['did'] = did2
cmp26['wid'] = wid1
url26 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations/" + did2
...
#############
chk26 = False
if out26['did'] == cmp26['did'] and out26['wid'] == cmp26['wid'] and out26['count'] == cmp26['count'] and out26[
    'status'] == cmp26['status'] and len(out26['tickets']) == len(cmp26['tickets']):
...
f.close()
data27 = {'data': json27}
url27 = "http://localhost:8080/thalia/tickets/donations"
...
cmp28['wid'] = wid1
cmp28['tickets'] = [tid2]
url28 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations/" + did1
...
################
chk28 = False
if out28['did'] == cmp28['did'] and out28['wid'] == cmp28['wid'] and out28['count'] == cmp28['count'] and out28[
    'status'] == cmp28['status'] and len(out28['tickets']) == len(cmp28['tickets']):
...
cmp29['wid'] = wid1
cmp29['tickets'] = [tid3]
url29 = "http://localhost:8080/thalia/shows/" + wid1 + "/donations/" + did2
...
################
chk29 = False
if out29['did'] == cmp29['did'] and out29['wid'] == cmp29['wid'] and out29['count'] == cmp29['count'] and out29[
    'status'] == cmp29['status'] and out29['tickets'][0] == cmp29['tickets'][0]:
...
url30 = join("http://localhost:8080/thalia/orders", oid1)
...
##########
chk30 = False
if cmp30['wid'] == out30['wid'] and cmp30['oid'] == out30['oid'] and cmp30['date_ordered'] == out30['date_ordered'] and \
                cmp30['order_amount'] == out30['order_amount']:
...
with open("cmp_files/out31.txt") as f:
    cmp31 = json.load(f)
f.close()
url31 = "http://localhost:8080/thalia/reports"
...
url32 = "http://localhost:8080/thalia/reports/" + mrid2 + "?show=" + wid1
...
##########
chk32 = False
if cmp32['mrid'] == out32['mrid'] and cmp32['name'] == out32['name'] and cmp32['total_shows'] == out32[
    'total_shows'] and cmp32['total_seats'] == out32['total_seats'] and cmp32['overall_revenue'] == out32[
    'overall_revenue']:
    ...
    if shows_o32['wid'] == shows_c32['wid']:
        ...
        if chk32:
            ...
            for a_, b_ in zip(sections_o32, sections_c32):
                if a_['sid'] != b_['sid'] or a_['section_name'] != b_['section_name'] or a_['section_price'] != b_[
                    'section_price'] or a_['seats_available'] != b_['seats_available'] or a_['seats_sold'] != b_[
                    'seats_sold'] or a_['section_revenue'] != b_['section_revenue']:
                    chk32 = False
                    break
                chk32 = True
                ############
...
url33 = "http://localhost:8080/thalia/search?topic=order&key=" + oid1
...
if orders_o33['oid'] == orders_c33['oid'] and orders_o33['wid'] == orders_c33['wid'] and orders_o33['date_ordered'] == \
        orders_c33['date_ordered'] and orders_o33['order_amount'] == orders_c33['order_amount'] and orders_o33[
    'number_of_tickets'] == orders_c33['number_of_tickets']:
